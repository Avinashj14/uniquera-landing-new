...
function uniqueraSignalConsultationFormSubmit(payload) {
    ...
    try {
        ...
        window.dataLayer.push(Object.assign({}, {
            event: eventName,
            form_id: 'uniquera_consultation_form_short',
            form_name: 'Uniquera short consultation form',
            form_flow_version: 6,
...
function uniqueraResolveThankYouRedirectUrl() {
    ...
    var dom = '';
    try {
        var el = document.getElementById('uniquera-react-form-short-root')
            || document.getElementById('uniquera-react-form-root');
...
(function ($) {
    ...
    $.fn.onlineFormShort = function (options) {
        ...
        return this.each(function () {
            ...
            var loading,
                ...
                $root = (function () {
                    var $r = $container.closest('#onlineForm, #onlineFormShort, [data-uniquera-form-shell]');
                    if (!$r.length) {
                ...
                /** Map full-form step numbers used in validation to short-form panel ids. */
                domQuestionNum = function (n) {
                    if (!isShortForm()) {
                        return n;
                    }
                    if (n === 10) {
                ...
                qSelector = function (n) {
                    return '#question-' + domQuestionNum(n);
                },
                ...
                formFlowVersion = function () {
                    return 6;
                },

                updateStepIndicator = function () {
                    var cur = typeof currentStep === 'number' && !isNaN(currentStep) && currentStep >= 1
                        ? currentStep
                        : NaN;
                    if (isNaN(cur)) {
                        var $a = $root.find('.steps .step.active');
                        cur = ($a.length && $a.text()) ? parseInt($.trim($a.text()), 10) : 1;
                    }
                    if (isNaN(cur) || cur < 1) {
                        cur = 1;
                    }
            ...
            stepCreate = function () {
                ...
                var totalSteps = lastQuestionNumber();
            ...
            doctorImage = function (questionNumber) {
                $root.find('.title-image [data-step]')
                    .removeClass('uniquera-title-visible')
                    .hide();
                $root.find('.title-image [data-step="' + questionNumber + '"]')
                    .addClass('uniquera-title-visible')
                    .show();
            }
            ...
            function removeQ10PolicyErrLater() {
                setTimeout(function () {
                    $root.find(qSelector(10)).find('input').removeClass('error');
                }, 800);
            }
            ...
            submitConsultationFromStep9 = function () {
                ...
                var fileInput = $root.find(qSelector(10) + ' input[name="files"]')[0]
            ...
            validateStepFields = function (questionNumber) {
                ...
                if (isShortForm()) {
                    if (q === 1) {
                        var frontS = $root.find('input[name=front]:checked').val();
                        var backS = $root.find('input[name=back]:checked').val();
                        $root.find('#question-1').find('.row.validate').removeClass('error');
                        if (frontS == undefined) {
                            $root.find('#question-1').find('.row.validate').eq(0).addClass('error');
                            showValidationModal(['Please select your front hair loss level.']);
                            settings.validate = false;
                            return false;
                        }
                        if (backS == undefined) {
                            $root.find('#question-1').find('.row.validate').eq(1).addClass('error');
                ...
                if (q === 10) {
                    var clearStep10Err = function () {
                        setTimeout(function () {
                            $root.find(qSelector(10) + ' .uniquera-step9-field').removeClass('has-error');
                            $root.find(qSelector(10) + ' .uniquera-q9-english.validate, ' + qSelector(10) + ' .uniquera-q9-travel.validate').removeClass('error');
                            $root.find(qSelector(10) + ' .form-control').removeClass('error');
                            $root.find(qSelector(10) + ' input[name="consent_info_processing"], ' + qSelector(10) + ' input[name="consent_information_accuracy"], ' + qSelector(10) + ' input[name="consent_contact_channels"]').removeClass('error');
                        }, 1400);
                    };
                    ...
                    if ($root.find('input[name=speaks_english]:checked').length === 0) {
                        $root.find(qSelector(10) + ' .uniquera-q9-english.validate').addClass('error');
                    ...
                    if ($root.find('input[name=travel_istanbul]:checked').length === 0) {
                        $root.find(qSelector(10) + ' .uniquera-q9-travel.validate').addClass('error');
                    ...
                    if (!pol1) {
                        $root.find(qSelector(10) + ' input[name="consent_info_processing"]').addClass('error');
                        step10Errors.push('Please accept consent for information processing.');
                    }
                    if (!pol2) {
                        $root.find(qSelector(10) + ' input[name="consent_information_accuracy"]').addClass('error');
                        step10Errors.push('Please confirm information accuracy.');
                    }
                    if (!pol3) {
                        $root.find(qSelector(10) + ' input[name="consent_contact_channels"]').addClass('error');
                        step10Errors.push('Please accept contact via WhatsApp/Phone/Email.');
                    }

                    if (step10Errors.length) {
                        showValidationModal(step10Errors, isShortForm() ? 'Please complete Step 4' : 'Please complete Step 10');
            ...
            loadQuestion = function (currentQuestionNumber) {
                ...
                if (currentQuestionNumber >= 1) {
                    ...
                    if (!isShortForm() && currentQuestionNumber === 9) {
                        assembleContactFormPayload();
                ...
                if (!isShortForm() && currentQuestionNumber === 4) {
                ...
                var data = {
                    ...
                    "language": language,
                    "updatedAt": Date.now(),
                    "formFlowVersion": formFlowVersion(),
            ...
            loadQuestionBack = function () {
                ...
                if (!isShortForm() && aktifEkran === 6 && $root.find('input[name="experienced"]:checked').val() === 'no') {
                    prevNumber = 4;
                }
            ...
            loadContent = function () {
                ...
                if (unfinished != null) {
                    if (unfinished.formFlowVersion === formFlowVersion()) {
                        currentQuestion = parseInt(unfinished.currentQuestion, 10) || 0;
                        if (isShortForm() && currentQuestion > 4) {
                            currentQuestion = 4;
                        }
                    } else if (unfinished.formFlowVersion === 5 && !isShortForm()) {
                        currentQuestion = parseInt(unfinished.currentQuestion, 10) || 0;
            ...
            var toggleSpeaksEnglishOtherInput = function () {
                ...
                var $otherWrap = $root.find(qSelector(10) + ' .uniquera-se-other-wrap');
                var $otherInput = $root.find(qSelector(10) + ' input[name=speaks_english_other]');
            ...
            var applyRadioSelection = function ($radio) {
                ...
                gender = $root.find('input[name=gender]:checked').val()
                    || $root.find('input[name=gender]').val();
            ...
            $root.on('focus input', qSelector(10) + ' input[name=speaks_english_other]', function () {
            ...
            run = function () {
                ...
                $root.find('.uniquera-thankyou-screen').each(function () {
                    var $panel = $(this);
